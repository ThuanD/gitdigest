...
export function getWordcloudCacheKey(period, lang) {
  ...
  if (
    ...
    throw new Error(
      "Invalid cache key parameters: period and lang must be non-empty strings",
    );
  }
...
export function getWordcloudCache(period, lang) {
  const cache = safeJsonParse(
    localStorage.getItem("wordcloud_cache") || "{}",
    {},
  );
...
export function invalidateWordcloudCacheByPattern(pattern) {
  try {
    ...
    if (removed > 0) {
      localStorage.setItem("wordcloud_cache", JSON.stringify(cache));
      console.log(
        `Invalidated ${removed} cache entries matching pattern: ${pattern}`,
      );
    }
...
export function performCacheMaintenance() {
  try {
    ...
    console.log(
      `Cache maintenance: removed ${expiredRemoved} expired, ${sizeRemoved} oversized entries`,
    );
